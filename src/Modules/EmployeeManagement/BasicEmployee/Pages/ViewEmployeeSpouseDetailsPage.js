...
const ViewEmployeeSpouseDetailsPage = ({employeeId = 1}) => {
  const [spouse, setEmployeeSpouseObject] = useState()
  const dataFetchedRef = useRef(false);
  const fetchEmployeeSpouse = (employeeId) => {
      fetch(`http://localhost:8000/api/employee-spouse/${employeeId}`)
      .then((r) => r.json())
      .then((spouse) => {
        // console.log(spouse)
        setEmployeeSpouseObject(spouse)
      })
    }
  useEffect(() => {
      if (dataFetchedRef.current) return;
      dataFetchedRef.current = true;
      fetchEmployeeSpouse(employeeId);
  },[])
  const renderPage = () => {
    if (!spouse) {
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li>
                <div class="flex items-center">
                  ...
                  <p class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">View Employee Spouse Details</p>
                </div>
              </li>
            </ol>
          </nav>
        </div>
        <PageTitle title="View Employee Spouse Details"></PageTitle>
        {renderPage()} 
      </div>
    </>
    )  
...
export default ViewEmployeeSpouseDetailsPage